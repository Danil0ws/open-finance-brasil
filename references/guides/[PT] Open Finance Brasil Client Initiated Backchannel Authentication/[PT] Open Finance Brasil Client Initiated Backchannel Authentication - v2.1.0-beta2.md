# [PT] Open Finance Brasil Client Initiated Backchannel Authentication - v2.1.0-beta2
...
[OFB-FAPI-BR](data/references/guides/PT.md) – Open Finance Brasil Financial-grade API Security Profile
...
Os requisitos deste perfil são adicionais e restritivos em relação às especificações [CIBA-Core](https://openid.net/specs/openid-financial-api-ciba-ID1.html) e [FAPI-CIBA](https://openid.net/specs/openid-financial-api-ciba-ID1.html), e devem ser observados conjuntamente com [OFB-FAPI-BR](data/references/guides/PT.md).
...
-   Deve estar em conformidade com os requisitos de segurança aplicáveis de [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3) para servidores de autorização.
...
-   O Cliente não tenha recebido a notificação dentro do prazo esperado, considerando as características da rede e os requisitos de disponibilidade estabelecidos em [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3);
...
-   Deve considerar o valor de `requested_expiry` enviado pelo Cliente, desde que seja um número inteiro positivo.
    
-   Deve determinar o prazo de validade da requisição de autenticação CIBA, informado no parâmetro `expires_in`, da seguinte forma:
    
    -   quando o `requested_expiry` estiver presente e seu valor for inferior ou igual ao prazo máximo aplicável, deve atribuir ao `expires_in` o valor solicitado pelo Cliente;
        
    -   quando o `requested_expiry` estiver ausente ou seu valor for superior ao prazo máximo aplicável, deve atribuir ao `expires_in` o prazo máximo definido para a aprovação do consentimento ou do vínculo de dispositivo, conforme a regra específica do produto ou serviço.
        
-   Não deve atribuir ao `expires_in` valor superior ao prazo máximo estabelecido pela regra específica do produto ou serviço.
    
-   Deve recusar tentativas de uso de `auth_req_id` após o término de seu prazo de validade, retornando erro conforme definido em [CIBA-Core](https://openid.net/specs/openid-financial-api-ciba-ID1.html) e [FAPI-CIBA](https://bitbucket.org/openid/fapi/src/master/Financial_API_WD_CIBA.md).
...
-   Pode negar novos pedidos de autenticação em casos de suspeita de fraude ou por razões de segurança, de acordo com suas políticas internas e com [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3);
...
O Cliente confidencial que participa de fluxos CIBA no Open Finance Brasil deve atender aos seguintes requisitos adicionais aos estabelecidos em [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3) e [FAPI-CIBA](https://bitbucket.org/openid/fapi/src/master/Financial_API_WD_CIBA.md).
...
-   Deve suportar o uso de tokens de atualização (refresh tokens), em conformidade com [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3) e [FAPI-CIBA](https://bitbucket.org/openid/fapi/src/master/Financial_API_WD_CIBA.md).
...
-   Deve expor um endpoint de notificação compatível com o ping mode, devidamente protegido, de acordo com [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3), incluindo o uso de TLS conforme [BCP195](https://tools.ietf.org/html/bcp195) e autenticação mútua via certificado, conforme [RFC8705](https://tools.ietf.org/html/rfc8705);
...
-   Pode enviar o parâmetro `requested_expiry` para solicitar o prazo de validade da requisição de autenticação CIBA. Quando enviado, seu valor deve ser um número inteiro positivo.
    
-   Deve considerar o valor de `expires_in` retornado pelo Servidor de Autorização CIBA como a fonte de verdade para a validade da requisição CIBA e para o planejamento de qualquer fallback, incluindo o uso eventual do poll mode.
...
-   Deve manter atualizados seus metadados de registro, incluindo os endpoints necessários para o ping mode e qualquer outra informação exigida por OFB-FAPI-BR-DCR e [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3).
    

## **7\. Considerações de segurança**

Os participantes devem apoiar todas as considerações de segurança especificadas em todas as cláusulas e subcláusulas de [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3) e [FAPI-CIBA](https://bitbucket.org/openid/fapi/src/master/Financial_API_WD_CIBA.md), bem como as recomendações de segurança relacionadas ao OAuth 2.0 ([RFC6749](https://tools.ietf.org/html/rfc6749), [RFC6750](https://tools.ietf.org/html/rfc6750), [RFC6819](https://tools.ietf.org/html/rfc6819)), ao uso de JWS/JWT ([RFC7515](https://tools.ietf.org/html/rfc7515), [RFC7519](https://tools.ietf.org/html/rfc7519)) e ao uso seguro de TLS ([BCP195](https://tools.ietf.org/html/bcp195), [RFC8705](https://tools.ietf.org/html/rfc8705)).

## 8\. Considerações sobre compartilhamento de dados

Os participantes devem apoiar todas as considerações de compartilhamento de dados especificadas em [OFB-FAPI-BR](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/82051180/PT+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3).
