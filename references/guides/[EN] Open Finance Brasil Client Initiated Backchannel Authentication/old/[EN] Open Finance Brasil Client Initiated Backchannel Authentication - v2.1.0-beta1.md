...
-   Shall not consider the value of `requested_expiry` that may be sent by the Client in the CIBA request. If the parameter is received, the CIBA Authorization Server shall ignore it.
    
-   Shall determine the validity period of the CIBA authorization request (`expires_in`) based on the period defined for the approval of the consent or the device binding, according to the specific rule of the product or service.
    
-   Shall refuse attempts to use `auth_req_id` outside this validity period, returning an error as defined in [CIBA-Core](https://openid.net/specs/openid-financial-api-ciba-ID1.html) and [FAPI-CIBA](https://bitbucket.org/openid/fapi/src/master/Financial_API_WD_CIBA.md).
...
-   Shall not send the `requested_expiry` parameter in CIBA requests in the context of Open Finance Brasil.
