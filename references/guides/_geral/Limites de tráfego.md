...
Os limites, quando implementados, são definidos por _endpoint_ e por origem como segue:
...
-   APIs de Consentimento (_Consents_)
    
-   APIs de Recursos (_Resources_)
    
-   Grupo de APIs de Serviços
    

O erro HTTP _Status_ _Code_ 429 (_Too many requests_) passa a ser utilizado de forma exclusiva para pedidos de consumo que excedam os limites de TPM.
...
Fica definida a utilização do código HTTP _Status_ _Code_ 529 (_Site is overloaded_) para retorno quando atingido o limite de TPS estabelecido.

_Endpoints_ criados dentro do conceito de extensibilidade, sejam dentro de novas APIs ou em APIs existentes, não podem ser considerados para controle do limite global de transações simultâneas.
