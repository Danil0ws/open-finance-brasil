...
-   NO\_CREDENTIAL: quando o CPF (loggedUser) / CNPJ (businessEntity) não for cliente ou não possuir credencial válida/ativa para prosseguir no fluxo monitorado.
...
Identifica a natureza do solicitante (PESSOA\_NATURAL/PESSOA\_JURIDICA)​  
Deve ser preenchida baseado no tipo de pessoa responsável pelo consentimento. Deverá ser observado se '/data/businessEntity' estiver preenchido no payload, se estiver então preencher com "PESSOA\_JURIDICA", se não estiver então preencher com "PESSOA\_NATURAL"
...
Valores possíveis para consentimento: AWAITING\_AUTHORISATION, PARTIALLY\_ACCEPTED, AUTHORISED, REJECTED, REVOKED, CONSUMED  
