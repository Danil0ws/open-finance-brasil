# [EN] Guia do Usuário para Entidades Transmissoras de Dados (ASPSP)
...
-   For transmiting/account holder institutions whose _Authorization Server_ supports more than one brand, it must accept more than one registry (client\_ids creation) for the same _software statement_. If your _Authorization Server_ implementation does not supports this behaviour, it must be suitable to support multiple brands and the registration of brands in the directory must be annotated according to each brand.
