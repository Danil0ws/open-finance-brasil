...
\*\*Tabela/Consulta\*\*: 'table\_name' ou 'custom\_query\_name'
...
| column\_1 | VARCHAR | \[Descrição\] | "Valor1", "Valor2" |

| column\_2 | INTEIRO | \[Descrição\] | 100, 200 |

| column\_3 | DATA | \[Descrição\] | 01-01-2024 |
...
| column\_1 | VARCHAR | \[Descrição\] | "Valor1" |

| column\_2 | DECIMAL | \[Descrição\] | 99,99 |

\### Relacionamentos/Unões

\`\`\`

Dataset1.key\_coluna → Dataset2.key\_coluna (Many-to-One)

[Dataset2.id](http://Dataset2.id) → Dataset3.foreign\_id (Um para Muitos)
...
SOMA(\[RECEITA\]) / CONTAGEM(DISTINTA \[customer\_id\])
...
\*\*Dependências\*\*: Requer campos 'receita' e 'customer\_id'
...
((SUM(\[current\_year\_sales\]) - SUM(\[previous\_year\_sales\]))

/ SUM(\[previous\_year\_sales\])) \* 100
...
-   \[metric\_field\_1\]: Agregação = SUM
    

-   \[metric\_field\_2\]: Agregação = média média
    

-   \*\*Campos Calculados Utilizados\*\*:
    

-   \[calculated\_variable\_1\]
...
-   Dica de Deslize: Mostrar \[campo1\], \[campo2\], \[calculated\_field\]
...
-   \*\*Eixo X\*\*: \[date\_field\] (Granularidade: Diária)
...
-   \*\*Série/Análise\*\*: \[category\_field\]
    

-   \*\*Campos Calculados\*\*: \[calculated\_variable\_2\]
...
| Nome do Cliente | \[customer\_name\] | Nenhum | Texto | 200px |

| Vendas Totais | \[vendas\] | SUM | Moeda | 120px |

| Contagem de Ordens | \[order\_id\] | CONDE | Número | 100px |

| Valor Médio da Ordem | \[calculated\_variable\_3\] | - | Moeda | 120px |
...
-   \*\*Valor\*\*: \[calculated\_variable\_4\] (Crescimento Ano Ano %)
...
-   \*\*Componente 2\*\*: Filtro adicional para \[product\_line\] = "Premium"
...
| target\_value | Número | 100000 | Variável Calculada 2 | Comparação de gols |

| currency\_conversion | Número | 1.0 | Múltiplos | Taxa de conversão de moeda |
...
-   \*\*Indexação\*\*: Garantir índices em \[campo1, campo2, date\_field\]
...
-   \*\*Visualizações Materializadas\*\*: Use 'mv\_sales\_summary' em vez de tabelas brutas
...
-   \*\*Equipe de Vendas\*\*: Filtrar ONDE \[sales\_rep\_id\] = CURRENT\_USER\_ID
    

-   \*\*Gerentes Regionais\*\*: Filtrar ONDE \[região\] = USER\_REGION
...
2.  \[Edição 2\]: \[Descrição e solução alternativa\]
...
| 1.0 | YYY-MM-DD | \[Nome\] | Lançamento inicial |

| 1.1 | YYY-MM-DD | \[Nome\] | Adicionado Componente 4, filtros atualizados |

| 2.0 | YYY-MM-DD | \[Nome\] | Grande redesenho, nova fonte de dados |
...
\#### Consulta 1: Principais Dados de Vendas
...
SUM(sales\_amount) como total\_sales,

CONTAR (order\_id DISTINTAS) COMO order\_count

DE sales\_table

ONDE date\_field >= '2024-01-01'

Status AND = 'concluído'

AGRUPAR POR 1, 2, 3, 4, 5

\`\`\`

\#### Consulta 2: Dados Alvo
...
\#### Campos Calculados Comuns

1.  \*\*Taxa de Conversão\*\*: '(SUM(\[conversões\]) / SUM(\[visitas\])) \* 100'
    

2.  \*\*Valor Médio do Pedido\*\*: 'SOMA(\[receita\]) / CONTAGEM(DISTINTO \[order\_id\])'
    

3.  \*\*Taxa de Crescimento\*\*: '((SUM(\[atual\]) - SUM(\[anterior\])) / SOM(\[anterior\])) \* 100'
    

4.  \*\*Total Contínuo\*\*: 'WINDOW\_SUM(SOMA(\[quantidade\]))'
    

5.  \*\*Rank\*\*: 'RANK(SUM(\[vendas\]), 'desc')'
