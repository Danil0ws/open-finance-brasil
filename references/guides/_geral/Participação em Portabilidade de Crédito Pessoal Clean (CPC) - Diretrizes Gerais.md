# Participação em Portabilidade de Crédito Pessoal Clean (CPC) - Diretrizes Gerais
...
-   Viabilizar a **criação de agendas** (bilaterais e multilaterais) antes do _Go Live_, para antecipação e correção de eventuais problemas e dificuldades encontradas; 
...
-   **Instituições Credoras Originais**: Instituições que possuem a API de _Loans_ publicada e que possuem o produto de Crédito Pessoal Clean (CPC); 
...
Para compartilhamento de **dados em massa** dos usuários, utilizar CSV, conforme seguinte modelo de arquivo:

Os campos enviados, seja de forma individual ou em massa, devem necessariamente respeitar as seguintes regras abaixo. Caso contrário, os dados **não serão** enviados ou processados.
...
Apenas números, **sem** traços ou pontos
...
Conforme OrgID da instituição no diretório, **com** traços
...
-   Para jornadas validadas (Etapa 1) **até 05/01/2026** 
...
-   Para jornadas validadas (Etapa 1) **a partir de 06/01/2026** 
...
-   A Caixa Econômica Federal realizará a abertura progressiva de sua funcionalidade para a base de clientes a partir de **7 de abril de 2026**. A abertura da base de clientes da proponente ocorrerá conforme as seguintes datas:
...
-   **UX**: Durante o período do piloto, serão realizadas validações de UX para as marcas participantes do Piloto. O questionário de UX, que será utilizado de base para validação das jornadas as instituições, será integralmente baseado nos requisitos do [Guia de Experiência do Usuário](https://openfinancebrasil.atlassian.net/wiki/pages/createpage.action?spaceKey=of&title=v.20.00.00%20Portabilidade%20de%20Cr%C3%A9dito&linkCreation=true&fromPageId=1171619917). 
...
-   **Engajamento - Portabilidades Concluídas**: Para cada instituição participante do piloto, um número mínimo de portabilidades deve ser concluído e identificado com sucesso via PCM: 
    
    -   **Para Proponentes**: 10 Portabilidades devem ser concluídas e identificadas com sucesso via PCM; 
        
    -   **Para Credoras**: 5 Portabilidades devem ser concluídas e identificadas com sucesso via PCM. As **instituições credoras** deverão, ao longo do período estabelecido, atingir os **critérios de engajamento,** conforme marcos descritos abaixo:
        ...
        -   Instituições que apresentem tickets bilaterais em aberto ou que não tenham atingido os marcos de engajamento poderão ser convocadas para **salas de guerra** conjuntas e bilaterais, previstos para ocorrerem de **13 a 17 de abril e de 4 a 15 de maio de 2026**.
...
As marcas das **Credoras Originais** participantes do Piloto serão divididas em 2 Grupos**:** 

-   **Grupo 1**: Marcas que o fornecedor possui conta aberta e possui o produto de CPC contratado para realização de validações com a instituição. 
    
    -   **FVP Manual**: Sucesso completo esperado nos 3 cenários de curta duração e em um cenário de longa duração, sob responsabilidade de execução do fornecedor; 
        
    -   **Validações de UX**: Sucesso completo esperado no questionário de UX;
        
    -   **Engajamento:** Sucesso em ao menos 5 Portabilidades concluídas e identificadas com sucesso via PCM, sob responsabilidade das instituições. Caso aplicável, as instituições também poderão realizar a solicitação de portabilidade via fornecedor. 
        
-   **Grupo 2**: Demais Marcas, que o fornecedor não possui conta aberta ou não possui o produto de CPC contratado para realização de validações com a instituição. 
    
    -   **Execução da FVP**: Sucesso completo esperado nos 3 cenários de curta duração, sob responsabilidade de execução da própria instituição; 
        
    -   **Validações de UX**: Sucesso completo esperado no questionário de UX. A jornada em ambiente produtivo deverá ser gravada pela instituição e disponibilizada à Associação para validação. 
        
    -   **Engajamento:** Sucesso em ao menos 5 Portabilidades concluídas e identificadas com sucesso via PCM, sob responsabilidade das instituições. As instituições do Grupo 2 não poderão realizar a solicitação de portabilidade via fornecedor. 
...
-   Que não atingirem os critérios **até 2 semanas antes do final do piloto**: Serão realizados _War rooms_ para apresentação e acompanhamento de planos de ação; 
...
-   Com Jornadas disponíveis e validadas **após 06/01/26**: Não participam do Piloto e terão a funcionalidade disponível para toda a base de clientes após conclusão de processo de onboarding_;_ 
...
![image-20251205-164029.png](images/image-20251205-164029.png)

## **Validações de UX para Credoras**

Com o objetivo de realizarmos as validações da experiência de usuário (UX) da área de gestão de portabilidade de crédito das Credoras Originais, estão abaixo descritas as instruções para validação de **(a)** **Teste 1 - portabilidade canceladas ou em andamento; (b) Teste 2 - portabilidade concluída**.
...
1.  A jornada que será gravada pela instituição deverá utilizar um **aplicativo atualizado**, contemplando a última versão disponível e utilizando um usuário real em ambiente produtivo. Gravações realizadas em emuladores ou qualquer outro cenário que não representem um aplicativo em ambiente produtivo **não serão considerados.**
    
    1.  Para o caso de instituições que não tenham habilitação para gravação das jornadas do aplicativos, é solicitado que seja utilizado um segundo aparelho para gravação da tela.
        
2.  Para validação da área de gestão, o usuário deverá ter **ao menos um contrato de crédito ativo** (CPC) que possa ser utilizado para a jornada de portabilidade. Caso não exista, o usuário deverá:
    
    1.  Contratar um Crédito Pessoal Clean (CPC);
        
    2.  Garantir que o contrato esteja ativo e visível no aplicativo;
        
    3.  Tenha uma portabilidade concluída com sucesso, para casos do Teste 2 - Portabilidade concluída.
        
3.  Para validação das jornadas de UX, é necessário que tenham
    
    1.  **Portabilidade cancelada ou em andamento** (Teste 1): É necessário que uma portabilidade esteja “Cancelada” ou “Em andamento”. Para que uma portabilidade esteja em qualquer um desses dois status, é possível seguir as seguintes alternativas:
        
        1.  **Via FVP:** O sucesso em qualquer um dos testes da FVP garantirá que uma portabilidade será cancelada, podendo seguir para a gravação da jornada do Teste 1 (Etapa 3 “Gravação das jornadas”).
            
        2.  **Via Proponente**: Funcionalidades de portabilidade de proponentes também podem ser utilizadas para que uma portabilidade vá para o status de “Em andamento” ou “Cancelada”.
            
    2.  **Portabilidade concluída** (Teste 2): É necessário que uma portabilidade esteja “Concluída”, necessariamente utilizando-se a funcionalidade de portabilidade de uma proponente. A gravação da jornada pode ser realizada por qualquer usuário, seja pelos próprios funcionários das instituições ou também pela base de clientes real. Instruções para obtenção de acesso e utilização da funcionalidade da proponente estão descritas nas seções “Público participante do piloto e restrição de usuários“ e “Instruções para utilização da Proponente - Caixa“.
...
-   **Contrato elegível em outra instituição:** Por fim, o usuário deve ter um contrato de Crédito Pessoal Clean (CPC) ativo em outra instituição financeira. Além de estar ativo, esse contrato precisa atender a alguns **critérios mínimos de elegibilidade** para a portabilidade ser possível via Caixa: 
    
    -   **Saldo devedor maior que R$ 150,00:** Contratos com saldos abaixo não são elegíveis. 
        
    -   **Valor da parcela mensal acima de R$ 20,00:** Parcelas de valor muito baixo não se qualificam. 
...
**A contratação do fornecedor refere-se especificamente à realização de solicitação de uma portabilidade de crédito** com uma Proponente de C**r**édito, em instituições nas quais seja possível a contratação de crédito pelo fornecedor, e **não garante a efetivação  da portabilidade**, que depende da apresentação de proposta pela Proponente e da identificação da liquidação pela PCM.
...
-   **Falha na etapa de compartilhamento de dados:** será cobrado 50% do va**l**or do teste (R$ 560,29) e a instituição poderá solicitar uma nova execução através do mesmo ticket;
...
-   **Agendas semanais**: Toda sexta-feira, das 14h às 15h, de 31/10/25 a 27/02/26   
    ...
    -   [Clique aqui](https://teams.microsoft.com/l/meetup-join/19%3ameeting_ZGRhMjYzN2ItNmNiMi00MTEwLWE2NDctMDRmYTUwMDIzMTI3%40thread.v2/0?context=%7b%22Tid%22%3a%228f7a0064-0d00-49c7-b6b8-33cd71b65429%22%2c%22Oid%22%3a%22b498dfdc-18bb-41af-88bc-ba08a755479f%22%7d) para entrar na agenda semanal de acompanhamento
        
-   **Grupo do** _**Whatsapp**_: Para dúvidas pontuais e interações com outras instituições ou Associação 
    
    -   [Clique aqui](https://chat.whatsapp.com/HKK9o212zs3CivEh9nAAQF) para acessar o Grupo do _Whatsapp_ do Piloto de Portabilidade de Crédito, ou escaneie o QR Abaixo com a câmera de seu celular:
        

![image-20251017-135316.png](images/image-20251017-135316.png)

## **Outros links úteis**

-   Descrição dos testes da FVP Manual: [Link aqui](https://gitlab.com/raidiam-conformance/open-finance/certification/-/wikis/Credit-Portability-CPC-API?redirected_from=Credit-Portability-API)
