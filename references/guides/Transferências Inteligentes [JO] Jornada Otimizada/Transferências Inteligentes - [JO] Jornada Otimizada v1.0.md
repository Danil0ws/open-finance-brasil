...
6.  O client solicita um novo _access\_token_ que inclua os escopos referentes a **contas** e **transferências inteligentes**.
...
**Obrigatório**: openid consent:consentId consents resources accounts recurring-consent:recurringConsentId recurring-payments
...
-   O **Software Client (C)** solicita ao **Authorization Server (AS)** um **access\_token técnico** via `POST /oauth/token` usando `grant_type=client_credentials`.
...
-   O Cliente envia `POST /consents` na **API Consents (D)** com:
    
    -   **permissions** restritas a saldos e limites (`ACCOUNTS_READ`, `ACCOUNTS_BALANCES_READ`, `ACCOUNTS_OVERDRAFT_LIMITS_READ`, `RESOURCES_READ`).
...
-   Regras:
    ...
    -   Caso contenha permissões fora do escopo, deve retornar **422 -** `COMBINACAO_PERMISSOES_INCORRETA`.
...
-   O Cliente envia `POST /recurring-consents` na **API TI** com:
...
-   Regras:
    ...
    -   O vínculo é estabelecido através do `linkId`.
...
-   Caso o usuário aprove:
    ...
    -   O Cliente troca via `POST /oauth/token` (`grant_type=authorization_code`).
        
    -   Retorno: `access_token` (com escopos combinados) e `refresh_token`.
        

### **7\. Descoberta de Recursos e Consulta de Dados**

-   O Cliente usa o `access_token` para:
    
    -   `GET /resources` → lista de recursos autorizados.
        
    -   `GET /accounts` → lista de contas vinculadas.
        
    -   `GET /accounts/{accountId}/balances` → saldos e limites da conta.
...
-   O **AS** verifica a validade dos consentimentos (TI e Dados).
    
    -   Se ao menos um consentimento ainda estiver válido → retorna novo `access_token`.
        
    -   Se ambos expiraram/revogados → retorna erro `invalid_grant`.
...
-   **Revogação do Consentimento de TI (Primário)**:
    ...
    -   Usuário envia `PATCH /recurring-consents/{recurringConsentId}`.
        
    -   Revoga **automaticamente** o consentimento de Dados vinculado.
